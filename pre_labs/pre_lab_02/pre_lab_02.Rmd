...
**Answer** It allows us to keep track of that filtered data so we can return to it and keep using it, instead of constantly filtering for it over and over.
...
**Answer** Shows the total number of theatre classes combined with classes with over 15 seats regardless of whether or not they're in the theatre department.
...
**Answer:** The expenditure date has been converted into a <date> column, which R can interpret.
...
**Answer** A new column has been made grouping the dates into months. The values show which months had a larger amount of money paid during them.
...
**Answer** Creating two new columns, one with the total number of votes cast in the gubernatiorial election and another with the total percentage of votes Wes Moore received out of the total votes.
...
**Answer** It arranged the data from highest percentage of votes to Moore to lowest percentage.
...
**Answer** This data arrangement is as useful as the previous arrangement, showing the same percentages of votes to Moore in a different order.
...
**Answer** str_to_lower() converts character columns to lowercase. str_to_title() converts character columns to titlecase, meaning only the first letter of each word is capitalized.
...
**Answer** The code is combing through the addresses in the data looking for the specific states we specified and applying each abbreviation to the rows of data with these specific terms.
...
**Answer** The way the data is grouped here shows the amount of expenditures from the states that were specified in the previous code block. It makes sense that Maryland would have the highest number of expanses by buying from in-state payees.
...
1. Which payee from Maryland was given the most amount of money?

2. Which expense purpose was the most frequently cited?

3. How many journalism classes mention video or videography in the title?
