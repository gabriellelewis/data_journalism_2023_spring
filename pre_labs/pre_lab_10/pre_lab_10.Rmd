...
**Answer** After loading the data, the output shows that this dataframe has 8 columns: OBJECTID, ZIPCODE2, ZIPNAME, EXISTING, ZIPCODE1, Shape_Leng, Shape_Area and geometry.
...
**Answer** Running the code below takes me to the dataframe in R. For the first seven columns, the rows contain information similar to things we've seen previously, such as numbers in the double columns and text in the character columns. However, for the last column 'geometry,' the values all start with MULTIPOLYGON followed by several parentheses and a set of numbers.
...
**Answer** The output is a map showing Maryland and the shapes of all the individual zip codes in the state.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The output just shows that the package has been installed.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The output shows this dataframe has 3,234 rows and 18 columns, each named for a specific characteristic about each county, such as name and state. The last column is also a geometry column.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The number of counties shown has shrunk to only show counties whose state FP number is 24.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The output is a map of Maryland showing the boundaries for each county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The map below now shows both the boundaries of each zip code in Maryland and the boundaries of each county. Some of these boundaries are overlapping one another.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The dataframe has a column containing zip codes and a column with the number of foreclosure notices for each zip code from March 2023.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** This dataframe now contains all the columns from md_zips, plus the number of foreclosures for each zip code from foreclosure_zip_march_2023.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The map below shows the boundaries of Maryland zip codes that are shaded according to the number of foreclosure notices they had in March 2023.
...
**Answer** The output just shows the libraries have been loaded.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Two new dataframes have been made. The md_county_notices dataframe shows each county and the number of foreclosure notices each one had in March. The md_county_population dataframe shows each county and their estimated populations.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The md_county_population dataframe now has a geometry column.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The dataframes have different names for each county as stated above, with the population dataframe saying "[county name], Maryland" while the foreclosure dataframe just says "[county name]".
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The names in the population dataframe have been fixed to just say the county name and not have ", Maryland" attached at the end.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** A new dataframe has been made that combines the population and foreclosure notice dataframes.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** A new column has been made that calculates the number of foreclosure notices per 1000 population, and the data is arranged so the highest rates are listed first.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The map shows each county in Maryland shaded by the number of foreclosure notices it has.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The color scale has been changed to make it easier to discern counties from one another and their rates.
...
**Answer** The color scale has been further adjusted to make it easier to tell apart counties and their rates. Based on the output below, Charles County has the highest rate of foreclosure notices per 1000. The areas with these high notices tend to be counties next to one another, such as Prince George's County and Calvert County.
