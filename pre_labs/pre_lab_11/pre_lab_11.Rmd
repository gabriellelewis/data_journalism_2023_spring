...
**Answer** The columns have names in regular titlecase with special characters like /, and the date column itself doesn't have the dates written in a way R can recognize.
...
**Answer** It'd be useful to look at if the release was deleted/removed or not, as well as the office(s) that authored the releases.
...
**Answer** The output pulled all of the 'li' tags from the osp_results variable. The output on my computer doesn't show the full list of all the 'li' elements, but it does suggest there is an immense number.
...
**Answer** For each 'li' item, there's now an output of what text is written within that li item.
...
**Answer** The dataframe now just have one column called "value" with the html_text output for each row, or 'li' item.
