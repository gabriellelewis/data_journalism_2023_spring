...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer** Records for certain years are uncategorized. Some of the categories also aren't actual names, such as the category L.
...
**Answer** The smallest grant amount is only $60, which is very low compared to many of the other grant values.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer** The headers are all standardized now as "one word" by turning two-word columns into all lowercase with _ replacing the spaces. One word columns are also all lowercase now.
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** 25 duplicate rows were deleted. I didn't see the previous task and ran this before I checked for duplicates. I'm basing the number of duplicates off the difference between the rows listed when running this data before cleaning up the dupes to the rows listed after cleaning up the dupes.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** This displays rows that qualify by displaying 5-character zip codes.
