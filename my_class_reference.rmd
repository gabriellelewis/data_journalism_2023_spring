...
Lubridate -> helps working with date-times and time-spans, can mutate columns into dates that R can interpret, can create new columns from data
  * functions: mdy, ymd, ymd_hms, floor_date (groups all dates into the first one mentioned of a specific month/year), str_squish() to clean up any       extra space, str_replace_all(column name, 'thing to replace', 'replacement'), wday(df$date column, label = TRUE) to get weekday labels from         date column, bind_rows(list(dataframe 1, dataframe 2, etc.)) to combine files with IDENTICAL structures of data, 
      left_join(added dataframe, by ="column name")
