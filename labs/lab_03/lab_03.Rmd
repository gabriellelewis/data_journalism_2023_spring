...
A3. To clean the data so nearly identical vendors are combined, my first thought is to mutate these vendors' names and use the case_when() function. Examples of vendors with name variations throughout the data are Plano-Coudon, LLC (variation of Plano-Coudon LLC), Cannon Design (variation of Cannon Design Inc and Cannon Washington Inc) and Olympus America Inc. (variation of Olympus America Inc). For vendors like these, the case_when() function would show if a row contains key terms found in all the variations — for example, both Olympus America Inc. variations have "Olympus America Inc" no period — in that case, they would both be given the same name to make the identical. I'd also use the distinct() function to get rid of duplicate rows.
