...
```{r}
baltimore_county_OD_calls <- read_csv("data/tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) %>%
  clean_names() %>%
  rename(date = x1, time = x2, case_number = x3, ev_type = x4, location = x5) %>%
  mutate(date=mdy(date))

baltimore_county_OD_calls
```
...
A1. The most overdose calls were made on July 14, 2022, and October 4, 2022, with both days having 23 calls. After going through all the rows summarized, it seems like there were not overdose calls made from December 20, 2022, to December 24, 2022. These dates are not present in the data, and I double checked by going back to the actual dataframe and searching for those dates, and I didn't find them.
...
A2. There doesn't seem to be one weekday that stands out among the rest as the one that gets the most amount of calls. Based on the small differences in percentages, Saturday gets the most amount of calls at 15.48%, and Thursday gets the least amount of calls at 12.79%, but again, these differences are pretty small.

```{r}
baltimore_county_OD_calls <- baltimore_county_OD_calls %>%
  mutate(weekday = wday(baltimore_county_OD_calls$date, label=TRUE))
...
baltimore_county_OD_calls %>%
  ...
  mutate(pct_weekday = (count_weekday/4075)*100)
...
A3. The location with the most amount of calls is 4540 Silver Spring Road, but looking at the location on Google Maps, there isn't a clear building to attribute to the address. It's right at a traffic light crossroads. The next two locations with the highest number of overdose calls are police stations. Some of the addresses are written in ways that don't actually reflect street names, such as "115 Susquehanna Av W". I can infer this means West Susquehanna Avenue because that's the closest name match in Maryland, but there may be other instances where the addresses on this data aren't exact matches to the real location. I'm more hesitant to trust the counts by location.
...
A4. I'm curious about if 4540 Silver Spring Road's actual location is the traffic stop I found on Google Maps, and if so, I'm curious as to why this location gets so many more calls than any of the other locations in this dataframe. It's received almost 20 more calls than its runner-ups.
